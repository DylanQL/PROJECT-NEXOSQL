...
const getUserConnections = async (req, res) => {
  try {
    ...
    return res.status(200).json(conexionesSinPassword);
  } catch (error) {
    console.error('Error fetching user connections:', error);
    return res.status(500).json({ error: 'Error al obtener las conexiones del usuario' });
  }
};
...
const createConnection = async (req, res) => {
  try {
    const { nombre, motores_db_id, host, port, username, password, database_name } = req.body;
    const userId = req.user.id;

    // Validate required fields
    if (!nombre || !motores_db_id || !host || !port || !username || !password || !database_name) {
...
const updateConnection = async (req, res) => {
  try {
    const { id } = req.params;
    const { nombre, motores_db_id, host, port, username, password, database_name, estado } = req.body;
    const userId = req.user.id;
