...
import { Container, Row, Col, Form, Button, Card, Spinner, Alert, Dropdown, InputGroup } from 'react-bootstrap';
...
const ChatInterface = () => {
  const { connections, activeConnection, loading: connectionsLoading, error: connectionsError } = useConnection();
  ...
  const handleChatRenamed = (chatId, newTitle) => {
    try {
      const updatedChat = chatService.renameChat(activeConnection.id, chatId, newTitle);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!activeConnection || !selectedChatId || !userInput.trim() || isProcessing) {
      return;
    }
    ...
    try {
      ...
      await chatService.sendQuestion(activeConnection.id, selectedChatId, userInput.trim());

      // Update the chats from storage to get the updated messages
      setChats(chatService.getChats(activeConnection.id));
      setUserInput('');
    } catch (error) {
      setError(`Error al procesar tu consulta: ${error.message || 'Por favor, intenta de nuevo.'}`);
      console.error(error);
    } finally {
        ...
        return (
          <div className="chat-message assistant-message">
            <div className="message-content bg-light p-3 rounded-3">
              ...
              {message.metadata && message.metadata.queries && message.metadata.queries.length > 0 && (
                <div className="mt-3 pt-3 border-top">
                  <details>
                    <summary className="text-muted fw-bold cursor-pointer">
                      Consultas SQL ejecutadas ({message.metadata.queries.length})
                    </summary>
                    <div className="mt-2">
                      {message.metadata.queries.map((query, index) => (
                        <div key={index} className="mb-2">
                          <div className="bg-dark text-light p-2 rounded">
                            <pre className="mb-0"><code>{query.sql}</code></pre>
                          </div>
                          {query.error && (
                            <Alert variant="danger" className="mt-1 mb-2 py-1 px-2">
          ...
          {!selectedChat ? (
            <div className="flex-grow-1 d-flex flex-column justify-content-center align-items-center bg-light text-center p-4">
              ...
              <p className="text-muted">
                Selecciona un chat existente o crea uno nuevo para comenzar a interactuar con tus bases de datos a través de lenguaje natural.
              </p>
              {activeConnection && (
            ...
            <>
              ...
              <div className="p-3 border-bottom">
                ...
                <small className="text-muted">
                  Conexión: {activeConnection?.nombre} ({activeConnection?.motor?.nombre})
                </small>
              </div>
              ...
              <div className="flex-grow-1 p-3 overflow-auto bg-light">
                ...
                {selectedChat.messages.length === 0 ? (
                  <div className="text-center my-5 py-5">
                    ...
                    <p className="text-muted">
                      Puedes preguntar cosas como "¿Cuántos usuarios hay registrados?" o "Muéstrame las ventas del último mes"
                    </p>
                  </div>
                ) : (
                ...
                <Form onSubmit={handleSubmit}>
                  <InputGroup>
                    ...
                    <Button
                      type="submit"
                      variant="primary"
                      disabled={isProcessing || !userInput.trim() || !activeConnection}
                    >
                      {isProcessing ? (
